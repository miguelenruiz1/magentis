<div class="wrap">
    <b>
        <h1>Configuración de Catálogo Magentis</h1>
    </b><br><br><br>
    <?php
    ...
    if (isset($_POST['actualizar_catalogo'])) {
        // Lógica para consumir el servicio y crear entradas
        consumir_servicio_salesforce();
    }

    // Verificar si se ha enviado el formulario para eliminar todas las entradas
    if (isset($_POST['eliminar_entradas'])) {
        // Lógica para eliminar todas las entradas
        // Función para eliminar todas las entradas
        eliminar_todas_las_entradas();
    }

    // Verificar si se ha enviado el formulario para obtener imágenes y guardarlas en un JSON
    if (isset($_POST['actualizar_json_imagenes'])) {
        // Lógica para obtener imágenes y guardarlas en un JSON
        obtener_imagenes_paises_con_token();
    }

    ?>


    <!-- Nuevo formulario para importar y eliminar entradas -->
    <form method="post" action="">


        <!-- Agregar botón para actualizar el JSON de imágenes -->
        <input type="submit" name="actualizar_json_imagenes" class="button button-primary" value="Actualizar JSON de Imágenes"><br><br>


        <input type="submit" name="actualizar_catalogo" class="button button-primary" value="Importar Entradas desde Salesforce"><br><br>
        <input type="submit" name="eliminar_entradas" class="button button-primary" value="Eliminar Todas las Entradas">
...
// Función para consumir el servicio de Salesforce y consultar todos los productos
function consumir_servicio_salesforce()
{
    ...
    foreach ($resultados_totales as $dat) {
        ...
        $existing_entry = get_page_by_title($product_final, OBJECT, 'productos_platon');
...
// Añadir acción para manejar la ejecución del cron

add_action('ejecutar_consumo_salesforce', 'consumir_servicio_salesforce');
add_action('init', 'consumir_servicio_salesforce');
