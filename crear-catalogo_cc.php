...
function catalogo_magentis_activado()
{
    ...
    if (!$pagina_existente) {
        ...
        $pagina = array(
            ...
            'post_content'  => '[filtro_categorias]', // Agrega aquí el shortcode para mostrar el formulario
            'post_status'   => 'publish',
            'post_type'     => 'page',
        );
...
function mi_accion_ajax() {
    ...
    $sql = "SELECT DISTINCT p.ID, p.post_title, p.post_content FROM {$wpdb->posts} p";

    // Añade joins para taxonomías si hay categorías seleccionadas
    ...
    $sql .= " WHERE p.post_type = 'productos_platon' AND p.post_status = 'publish'";

    // Añade condiciones para las categorías seleccionadas
    if (!empty($selected_categories)) {
        $sql .= " AND (";
        $conditions = array();
        foreach ($selected_categories as $taxonomy => $terms) {
            if (is_array($terms)) {
                foreach ($terms as $term) {
                    $conditions[] = $wpdb->prepare("(tt.taxonomy = %s AND t.term_id = %d)", $taxonomy, $term);
                }
            } else {
                $conditions[] = $wpdb->prepare("(tt.taxonomy = %s AND t.term_id = %d)", $taxonomy, $terms);
            }
        }
    ...
    // Agrupa por título y contenido para obtener resultados únicos
    $sql .= " GROUP BY p.post_title, p.post_content";
    ...
    // Carga el archivo JSON para los datos de imagen
    ...
    foreach ($posts as $post) {
        ...
        // Asegúrate de que 'records' existe en $dataimgsal y tiene el formato esperado
    ...
    // Envía el resultado como una respuesta JSON
    wp_send_json($result);
    die();
}
